...
void CPlotter::resizeEvent(QResizeEvent* )                    //resizeEvent()
{
  ...
  if( m_Size != size() or (m_bReference != m_bReference0) or
    ...
    if(m_h2>m_h-15) m_h2=m_h-15;
    if(m_bReference) m_h2=m_h-15;
    if(m_h2<1) m_h2=1;
    m_h1=m_h-m_h2;
    ...
    m_2DPixmap.fill(Qt::black);
    m_ScalePixmap = QPixmap(m_w,15);
    m_ScalePixmap.fill(Qt::gray);
...
void CPlotter::paintEvent(QPaintEvent *)                                // paintEvent()
{
  ...
  painter.drawPixmap(0,15,m_WaterfallPixmap);
...
//draw tick marks on upper scale
  ...
  for( int i=0; i<m_hdivs; i++) {                    //major ticks
    ...
    painter0.drawLine(x,8,x,15);
  }
  int minor=5;
  if(m_freqPerDiv==200) minor=4;
  for( int i=1; i<minor*m_hdivs; i++) {             //minor ticks
    x = i*pixperdiv/minor;
    painter0.drawLine(x,12,x,15);
  }

  //draw frequency values
  for( int i=0; i<=m_hdivs; i++) {
    x = (int)((m_xOffset+i)*pixperdiv - pixperdiv/2);
    if(int(x+pixperdiv/2) > 30) {
      rect0.setRect(x,0, (int)pixperdiv, 8);
  ...
  if(m_mode=="JT4") {                        //JT4
    ...
    for(int i=0; i<4; i++) {
      ...
      int j=12;
      if(i==0) j=9;
      painter0.drawLine(x1,j,x1,15);
    }
    painter0.setPen(penRed);
    for(int i=0; i<4; i++) {
      x1=XfromFreq(m_txFreq+bw*i/3.0);
      painter0.drawLine(x1,9,x1,12);
    }
  }
  ...
  if(m_mode=="JT9" or m_mode=="JT65" or m_mode=="JT9+JT65" or m_mode=="QRA64" or m_mode=="FT8") {
    ...
    if(m_mode=="QRA64" or (m_mode=="JT65" and m_bVHF)) {
      ...
      if(m_mode=="JT65") {
        ...
        painter0.drawLine(x3,12,x3,15);
        x4=XfromFreq(m_rxFreq+30.0*bw/65.0);    //RRR
        painter0.drawLine(x4,12,x4,15);
        x5=XfromFreq(m_rxFreq+40.0*bw/65.0);    //73
        painter0.drawLine(x5,12,x5,15);
      }
      painter0.setPen(penGreen);
      x6=XfromFreq(m_rxFreq+bw);             //Highest tone
      painter0.drawLine(x6,12,x6,15);
      ...
      painter0.drawLine(x1,11,x1,14);
      painter0.drawLine(x1,14,x2,14);
      painter0.drawLine(x1,13,x2,13);
      painter0.drawLine(x2,11,x2,14);
    }
  }
  ...
  if(m_mode=="JT9" or m_mode=="JT65" or m_mode=="JT9+JT65" or
    ...
    //Modified by KD8CEC for RPI
    painter0.drawLine((x2 - x1) / 2 + x1, 5,(x2 - x1) / 2 + x1, 13);
    painter0.drawLine(x1,8,x2,8);
    painter0.drawLine(x1,9,x2,9);
    painter0.drawLine(x1,10,x2,10);
    //painter0.drawLine(x2,9,x2,11);
  }
