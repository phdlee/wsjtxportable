...
char volatile ft8msgbits[87];           //fixed by KD8CEC overflow memory, 75 + CRC = 87 //packed 75 bit ft8 message
...
qint64 orgFrequency = 7000000;
qint64 dispfrequency = 7000000;
qint32 freqMoveStep = 50;
qint64 vfoBfrequency = 21130000;
  ...
  m_manual {&m_network_manager}
{
  ...
  //for RPI (fixed size) by KD8CEC
  menuBar()->setParent(ui->frameMenu);
  //setWindowFlags (Qt::WindowCloseButtonHint | Qt::WindowMinimizeButtonHint| Qt::WindowStaysOnTopHint | Qt::FramelessWindowHint);
  setWindowFlags (Qt::WindowCloseButtonHint | Qt::WindowMinimizeButtonHint| Qt::FramelessWindowHint);
...
//  Q_EMIT startAudioInputStream (m_config.audio_input_device (), m_framesAudioInputBuffered, &m_detector, m_downSampleFactor, m_config.audio_input_channel ());
  ...
  //Append Lines by KD8CEC
  move(1, 100);
  this->setMaximumWidth(480);
  this->setMaximumHeight(220);
  this->setFixedSize(this->size());

  ui->stk1->setCurrentIndex(0);
  ui->frameStatus->setVisible(false);
  ui->btnCatConnect->setVisible(false);
  ui->lblStep->setText(QString::number(freqMoveStep) + "Hz");
  ui->lblVFOOther->setText(Radio::pretty_frequency_MHz_string(vfoBfrequency));

  ui->frmMemory->move(0, 0);
  ui->frmMemory->setVisible(false);

  ui->lblMM01->setText(Radio::pretty_frequency_MHz_string(stMM01));
  ui->lblMM02->setText(Radio::pretty_frequency_MHz_string(stMM02));
  ui->lblMM03->setText(Radio::pretty_frequency_MHz_string(stMM03));
  ui->lblMM04->setText(Radio::pretty_frequency_MHz_string(stMM04));
  ui->lblMM05->setText(Radio::pretty_frequency_MHz_string(stMM05));
  ui->lblMM06->setText(Radio::pretty_frequency_MHz_string(stMM06));
  ui->lblMM07->setText(Radio::pretty_frequency_MHz_string(stMM07));
  ui->lblMM08->setText(Radio::pretty_frequency_MHz_string(stMM08));
  ui->lblMM09->setText(Radio::pretty_frequency_MHz_string(stMM09));
  ui->lblMM10->setText(Radio::pretty_frequency_MHz_string(stMM10));
  ui->lblMM11->setText(Radio::pretty_frequency_MHz_string(stMM11));
  ui->lblMM12->setText(Radio::pretty_frequency_MHz_string(stMM12));
...
void MainWindow::writeSettings()
{
  ...
  //for QRP with Raspberry pi by KD8CEC
  m_settings->beginGroup ("RPI");
  m_settings->setValue("vfobfreq", vfoBfrequency);
  m_settings->setValue("freqmstep", freqMoveStep);

  m_settings->setValue("stmm01", stMM01);
  m_settings->setValue("stmm02", stMM02);
  m_settings->setValue("stmm03", stMM03);
  m_settings->setValue("stmm04", stMM04);
  m_settings->setValue("stmm05", stMM05);
  m_settings->setValue("stmm06", stMM06);
  m_settings->setValue("stmm07", stMM07);
  m_settings->setValue("stmm08", stMM08);
  m_settings->setValue("stmm09", stMM09);
  m_settings->setValue("stmm10", stMM10);
  m_settings->setValue("stmm11", stMM11);
  m_settings->setValue("stmm12", stMM12);
  m_settings->endGroup();
...
void MainWindow::readSettings()
{
  ...
  //for QRP with Raspberry pi by KD8CEC
  m_settings->beginGroup ("RPI");
  freqMoveStep = m_settings->value ("freqmstep", 100).toInt();
  vfoBfrequency = m_settings->value ("vfobfreq", 21130000).toLongLong();

  stMM01 = m_settings->value ("stmm01", 0).toLongLong();
  stMM02 = m_settings->value ("stmm02", 0).toLongLong();
  stMM03 = m_settings->value ("stmm03", 0).toLongLong();
  stMM04 = m_settings->value ("stmm04", 0).toLongLong();
  stMM05 = m_settings->value ("stmm05", 0).toLongLong();
  stMM06 = m_settings->value ("stmm06", 0).toLongLong();
  stMM07 = m_settings->value ("stmm07", 0).toLongLong();
  stMM08 = m_settings->value ("stmm08", 0).toLongLong();
  stMM09 = m_settings->value ("stmm09", 0).toLongLong();
  stMM10 = m_settings->value ("stmm10", 0).toLongLong();
  stMM11 = m_settings->value ("stmm11", 0).toLongLong();
  stMM12 = m_settings->value ("stmm12", 0).toLongLong();
  m_settings->endGroup ();
...
{
  return;   //by kd8cec for raspberry pi, fixed font size
  qApp->setFont (font);
...
//  int k (frames - 1);
  ...
  if (ui)
  {
      if (ui->pBarSignal->maximum() < m_pxmax)
      {
          ui->pBarSignal->setMaximum(m_pxmax);
      }

      ui->pBarSignal->setValue(m_px);
  }
...
void MainWindow::displayDialFrequency ()
{
  ...
  if (isSetFreqMode == 0)
  {
    ui->labDialFreq2->setText (Radio::pretty_frequency_MHz_string (dial_frequency));
    lastDisplayFreq = dial_frequency;
  }

  update_dynamic_property (ui->lblARX, "istx", m_rigState.ptt());
  update_dynamic_property (ui->lblATX, "istx", m_rigState.ptt());

  if (m_rigState.mode() == Transceiver::LSB)
  {
      ui->lblMode->setText("LSB");
  }
  else if (m_rigState.mode() == Transceiver::USB)
  {
      ui->lblMode->setText("USB");
  }
  else if (m_rigState.mode() == Transceiver::CW)
  {
      ui->lblMode->setText("CW");
  }
  else
  {
      ui->lblMode->setText("");
...
{
  QFont tmpFont = tx_status_label.font();
  tmpFont.setPointSize(8);
  tx_status_label.setAlignment (Qt::AlignHCenter);
  tx_status_label.setMinimumSize (QSize  {130, 15});
  tx_status_label.setMaximumSize (QSize  {130, 15});
  tx_status_label.setStyleSheet ("QLabel{background-color: #00ff00}");
  tx_status_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  tx_status_label.setFont(tmpFont);
  statusBar()->addWidget (&tx_status_label);

  config_label.setAlignment (Qt::AlignHCenter);
  config_label.setMinimumSize (QSize {50, 15});
  config_label.setMaximumSize (QSize {50, 15});
  config_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  config_label.setFont(tmpFont);
  ...
  mode_label.setMinimumSize (QSize {50, 15});
  mode_label.setMaximumSize (QSize {50, 15});
  mode_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  mode_label.setFont(tmpFont);
  statusBar()->addWidget (&mode_label);

  last_tx_label.setAlignment (Qt::AlignHCenter);
  last_tx_label.setMinimumSize (QSize {130, 15});
  last_tx_label.setMaximumSize (QSize {130, 15});
  last_tx_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  last_tx_label.setFont(tmpFont);
  statusBar()->addWidget (&last_tx_label);

  band_hopping_label.setAlignment (Qt::AlignHCenter);
  band_hopping_label.setMinimumSize (QSize {0, 15});
  band_hopping_label.setMaximumSize (QSize {50, 15});
  band_hopping_label.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  band_hopping_label.setFont(tmpFont);
  band_hopping_label.setFixedWidth(0);
  statusBar()->addWidget (&band_hopping_label);

  statusBar()->addPermanentWidget(&progressBar, 1);
  progressBar.setMinimumSize (QSize {70, 13});
  progressBar.setMaximumHeight(15);
  progressBar.setFormat ("%v/%m");
  progressBar.setFont(tmpFont);

  statusBar ()->addPermanentWidget (&watchdog_label);
  statusBar()->setParent(ui->frameStatus);
...
void MainWindow::setup_status_bar (bool vhf)
{
  ...
  if (m_mode.contains (QRegularExpression {R"(^(Echo|ISCAT))"})) {
    //if (band_hopping_label.isVisible ()) statusBar ()->removeWidget (&band_hopping_label);
      if (band_hopping_label.width() > 5) band_hopping_label.setFixedWidth(0);
  } else if (m_mode.startsWith ("WSPR")) {
    mode_label.setStyleSheet ("QLabel{background-color: #ff66ff}");
    //if (!band_hopping_label.isVisible ()) {
      if (band_hopping_label.width() < 2) {
        band_hopping_label.setFixedWidth(50);
      //statusBar ()->addWidget (&band_hopping_label);
...
{
  //m_wideGraph->setGeometry(0, 0, 100, 100 );
  m_wideGraph->move(0, 0);
  m_wideGraph->show();
}

void MainWindow::on_actionEcho_Graph_triggered()
{
  m_echoGraph->move(0, 0);
  m_echoGraph->show();
}

void MainWindow::on_actionFast_Graph_triggered()
{
  m_fastGraph->move(0,0);
  m_fastGraph->show();
}
...
void MainWindow::hideMenus(bool checked)
{
  ...
  if (checked) {
      ...
      minimumSize().setHeight(320);
      minimumSize().setWidth(480);
...
qint8 catStatus = 0;    //0:init, 1 : Connected, 2 : fail
...
void MainWindow::handle_transceiver_update (Transceiver::TransceiverState const& s)
{
  ...
  update_dynamic_property (ui->lblACAT, "iscat", true);
  catStatus = 1;
  ui->btnCatConnect->setVisible(false);
}
...
void MainWindow::handle_transceiver_failure (QString const& reason)
{
  ...
  on_stopTxButton_clicked ();
  rigFailure (reason);
  update_dynamic_property (ui->lblACAT, "iscat", false);
  catStatus = 2;
  ui->btnCatConnect->setVisible(true);
}
...
void MainWindow::rigFailure (QString const& reason)
{
  if (m_first_error)
    {
      // one automatic retry
      //modify Line by KD8CEC
      QTimer::singleShot (3000, this, SLOT (rigOpen ()));
      m_first_error = false;
    }
  else
    {
      ...
      //Append Line by kd8cec for small lcd
      m_rigErrorMessageBox.move(1, 1);
...
void MainWindow::on_btnExit_clicked()
{
    ...
    msgBox.setInformativeText("Quits the program");
    ...
    */
    if (MessageBox::Yes == MessageBox::query_message (this
                                                      , tr ("Really Quit?")
                                                      , tr ("Quits the program"))) {
        QTimer::singleShot (0, this, SLOT (close ()));
    }
}

void MainWindow::on_btnVFO_clicked()
{
   ui->stk1->setCurrentIndex(0);
   ui->frameStatus->setVisible(false);
}

void MainWindow::on_btnRX_clicked()
{
    ui->stk1->setCurrentIndex(1);
    ui->frameStatus->setVisible(true);
}

void MainWindow::on_btnTX_clicked()
{
    ui->stk1->setCurrentIndex(2);
    ui->frameStatus->setVisible(true);
}

void MainWindow::on_btnOption_clicked()
{
    ui->stk1->setCurrentIndex(3);
    ui->frameStatus->setVisible(true);
}

Radio::Frequency inputFrequency = 0;
char isPressPoint = 0;

//0~9, 10 : Point, 11 : Enter, 12 : Cancel
void MainWindow::inputVFOFreq(int inputValue)
{
    if (isSetFreqMode == 0)	//Start Frequency Input
    {
        //Clear
        inputFrequency = 0;
        isPressPoint = 0;
    ...
    if (inputValue == 12)	//Cancel
    {
        //Clear
        inputFrequency = 0;
        isPressPoint = 0;
        isSetFreqMode = 0;
        update_dynamic_property (ui->labDialFreq2, "input", false);
        displayDialFrequency();
        return;
    }
    else if (inputValue == 10)  //Point
    {
	if (isPressPoint == 0 && inputFrequency < 1000)	//less than 1Ghz
	{
        isPressPoint = 1;
		inputFrequency *= 1000000;
	}
    }
    else if (inputValue >= 0 && inputValue <= 9 )
    {
        ...
        {
		inputFrequency *= 10;
		inputFrequency += inputValue;
        }
        else
        {
		if (isPressPoint == 1)
			inputFrequency += (inputValue * 100000);
		else if (isPressPoint == 2)
			inputFrequency += (inputValue * 10000);
		else if (isPressPoint == 3)
			inputFrequency += (inputValue * 1000);
		else if (isPressPoint == 4)
			inputFrequency += (inputValue * 100);
		else if (isPressPoint == 5)
			inputFrequency += (inputValue * 10);
		else if (isPressPoint == 6)
			inputFrequency += inputValue;
    ...
    if (inputValue == 11 || (isPressPoint >= 7) || (isPressPoint == 0 && inputFrequency >= 100000000))  //Enter / Send via CAT
    {
        isSetFreqMode = 0;
        setRig(inputFrequency);
        //displayDialFrequency();
    }
    else
    {
        ui->labDialFreq2->setText (Radio::pretty_frequency_MHz_string (inputFrequency));
    }

    update_dynamic_property (ui->labDialFreq2, "input", (isSetFreqMode == 1));
}
...
void MainWindow::on_btnA4_clicked()
{
    inputVFOFreq(4);
}

void MainWindow::on_btnA5_clicked()
{
    inputVFOFreq(5);
}

void MainWindow::on_btnA6_clicked()
{
    inputVFOFreq(6);
}

void MainWindow::on_btnA7_clicked()
{
    inputVFOFreq(7);
}

void MainWindow::on_btnA8_clicked()
{
    inputVFOFreq(8);
}

void MainWindow::on_btnA9_clicked()
{
    inputVFOFreq(9);
}
...
void MainWindow::on_btnAPoint_clicked()
{
    inputVFOFreq(10);
}

void MainWindow::on_btnAEnt_clicked()
{
    inputVFOFreq(11);
}

void MainWindow::on_btnACancel_clicked()
{
    inputVFOFreq(12);
}

void MainWindow::on_btnAUSB_clicked()
{
    //m_rigState.mode() == Transceiver::LSB
    Q_EMIT m_config.transceiver_mode(Transceiver::USB);
}

void MainWindow::on_btnALSB_clicked()
{
    Q_EMIT m_config.transceiver_mode (Transceiver::LSB);

}

void MainWindow::on_btnACW_clicked()
{
    Q_EMIT m_config.transceiver_mode(Transceiver::CW);
}

void MainWindow::on_btnAVfoAB_clicked()
{
    qint64 tmpAfrequency = 0;
    if (! ui->btnCatConnect->isVisible() &&  lastDisplayFreq != 0)
    {
        //Change Frequency
        //Save Frequency
        tmpAfrequency = lastDisplayFreq;
        lastDisplayFreq = vfoBfrequency;
        setRig(lastDisplayFreq);
        displayDialFrequency();
        vfoBfrequency = tmpAfrequency;
        ui->lblVFOOther->setText(Radio::pretty_frequency_MHz_string(vfoBfrequency));
    }
}

char nowMemoryType = 0; //0 : mem to vfo, 1 : vfo to mem

void MainWindow::on_btnAMem_clicked()
{
    ui->frmMemory->setVisible(true);
    nowMemoryType = 0;
    update_dynamic_property (ui->lblMemInfo, "vfotomem", nowMemoryType == 1);
}


void MainWindow::on_btnFreqStep_clicked()
{
    if (freqMoveStep == 1)
        freqMoveStep = 5;
    else if (freqMoveStep == 5)
        freqMoveStep = 10;
    else if (freqMoveStep >= 500)
        freqMoveStep = 1;
    else if (freqMoveStep >= 50)
        freqMoveStep += 50;
    else
        freqMoveStep += 10;
...
void MainWindow::on_btnAFreqDown_clicked()
{
    lastDisplayFreq -= freqMoveStep;
    setRig(lastDisplayFreq);
    displayDialFrequency();
}

void MainWindow::on_btnAFreqUp_clicked()
{
    lastDisplayFreq += freqMoveStep;
    setRig(lastDisplayFreq);
    displayDialFrequency();
...
void MainWindow::on_hSlider1_sliderPressed()
{
    isSetFreqMode = 1;
    orgFrequency = lastDisplayFreq;
}

void MainWindow::on_hSlider1_sliderMoved(int position)
{
    if (isSetFreqMode == 1)
    {
        dispfrequency = (orgFrequency + (position - 100) * freqMoveStep);
        ui->labDialFreq2->setText (Radio::pretty_frequency_MHz_string (dispfrequency));
    }
}

void MainWindow::on_hSlider1_sliderReleased()
{
    isSetFreqMode = 0;
    setRig(dispfrequency);
    //displayDialFrequency();
    ui->hSlider1->setValue(100);
}

void MainWindow::on_btnCatConnect_clicked()
{
    QTimer::singleShot (0, this, SLOT (rigOpen ()));
}

void MainWindow::on_btnAVfoToMem_clicked()
{
    nowMemoryType = nowMemoryType == 1 ? 0 : 1;
    if (nowMemoryType == 0)
        ui->lblMemInfo->setText("select the memory to load");
    else
        ui->lblMemInfo->setText("select the memory to save");

    update_dynamic_property (ui->lblMemInfo, "vfotomem", nowMemoryType == 1);

}

void MainWindow::on_btnMMCancel_clicked()
{
    ui->frmMemory->setVisible(false);
}

void MainWindow::mmMemoryProcess(int memIndex)
{
    qint64 tmpMMFreq = 0;
    ...
    else
    {
        ...
        {
            case 1 :
                stMM01 = lastDisplayFreq;
                break;
            case 2 :
                stMM02 = lastDisplayFreq;
                break;
            case 3 :
                stMM03 = lastDisplayFreq;
                break;
            case 4 :
                stMM04 = lastDisplayFreq;
                break;
            case 5 :
                stMM05 = lastDisplayFreq;
                break;
            case 6 :
                stMM06 = lastDisplayFreq;
                break;
            case 7 :
                stMM07 = lastDisplayFreq;
                break;
            case 8 :
                stMM08 = lastDisplayFreq;
                break;
            case 9 :
                stMM09 = lastDisplayFreq;
                break;
            case 10 :
                stMM10 = lastDisplayFreq;
                break;
            case 11 :
                stMM11 = lastDisplayFreq;
                break;
            case 12 :
                stMM12 = lastDisplayFreq;
                break;
            default :
                return;
        }
...
void MainWindow::on_btnMM01_clicked()
{
    mmMemoryProcess(1);
    //m_settings->setValue("stmm01", stMM01);
...
void MainWindow::on_btnMM02_clicked()
{
    mmMemoryProcess(2);
}

void MainWindow::on_btnMM03_clicked()
{
    mmMemoryProcess(3);
}

void MainWindow::on_btnMM04_clicked()
{
    mmMemoryProcess(4);
}

void MainWindow::on_btnMM05_clicked()
{
    mmMemoryProcess(5);
}

void MainWindow::on_btnMM06_clicked()
{
    mmMemoryProcess(6);
}

void MainWindow::on_btnMM07_clicked()
{
    mmMemoryProcess(7);
}

void MainWindow::on_btnMM08_clicked()
{
    mmMemoryProcess(8);
}

void MainWindow::on_btnMM09_clicked()
{
    mmMemoryProcess(9);
}

void MainWindow::on_btnMM10_clicked()
{
    mmMemoryProcess(10);
}

void MainWindow::on_btnMM11_clicked()
{
    mmMemoryProcess(11);
}

void MainWindow::on_btnMM12_clicked()
{
    mmMemoryProcess(12);
}
